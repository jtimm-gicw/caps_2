...
socket.on('connect', () => {
  socket.emit('SUBSCRIBE', { queueId: 'flowers' });
  console.log('1-800-flowers subscribed to flowers queue');
});
...
const generateOrder = (socket, payload = null) => {
  ...
  console.log('VENDOR: A flower order is ready for pickup.');
...
const thankDriver = (payload) => {
  console.log('Thanks for delivering the flowers to', payload.customer);
};
